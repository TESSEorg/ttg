...
struct ijT {
    ...
    template <typename Archive> void serialize(Archive& ar) {ar & i & j;}
...
public:
    ...
    rangeT& operator++() {lo+=1; return *this;}
...
public:
    ...
    MatrixBlock(ijT ijblock, const rangeT& irange, const rangeT& jrange)
        : meta(ijblock, irange, jrange)
        , d(meta.nelem, 0.0)
    {}

    // Constructor initializes data to zero
    MatrixBlock(const MatrixBlockMetaData& meta) 
        : meta(meta)
        , d(meta.nelem, 0.0)
    {}

    double& operator()(size_t i, size_t j) {return d[(i-meta.irange.start())*meta.jblockdim + (j-meta.jrange.start())];}
            
    double operator()(size_t i, size_t j) const {return d[(i-meta.irange.start())*meta.jblockdim + (j-meta.jrange.start())];}
    ...
    template <typename Archive> void serialize(Archive& ar) {ar & meta & d;}
};
...
struct MatrixMetaData {
    ...
    MatrixMetaData(size_t idim,
                   size_t jdim,
                   const std::vector<size_t>& ilo,
                   const std::vector<size_t>& jlo,
                   const std::shared_ptr<WorldDCPmapInterface<ijT>>& pmap = matrix_default_pmap)
        : idim(idim)
        , jdim(jdim)
        , iblockdim(ilo.size()-1)
        , jblockdim(jlo.size()-1)
        , ilo(ilo)
        , jlo(jlo)
        , pmap(pmap)
    {}
...
public:
    ...
    Matrix(World& world, const MatrixMetaData& meta)
        : meta(meta)
        , a(world, meta.pmap)
    {
        zero();
        ...
        for (auto iblock : rangeT(0,meta.iblockdim)) {
            for (auto jblock : rangeT(0,meta.jblockdim)) {
                ...
                if (a.is_local(ijblock)) {
                    ...
                    acc->second = MatrixBlock(ijblock, rangeT(meta.ilo[iblock],meta.ilo[iblock+1]), rangeT(meta.jlo[jblock],meta.jlo[jblock+1]));
                }
            }
        }
    }
    ...
    void add(const MatrixBlock& b) {
        a.send(b.ijblock(), &MatrixBlock::add, b);
    }
...
void BuildFockTask(const MatrixBlock& Dij,
                   const MatrixBlock& Dik,
                   const MatrixBlock& Dil,
                   const MatrixBlock& Djk,
                   const MatrixBlock& Djl,
                   const MatrixBlock& Dkl,
                   MatrixBlock& Jij, 
                   MatrixBlock& Jkl,
                   MatrixBlock& Kik,
                   MatrixBlock& Kil,
