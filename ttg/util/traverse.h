...
namespace ttg {
  ...
  namespace detail {
    ...
    class Traverse {
      std::set<base::OpBase *> seen;

      bool visited(base::OpBase *p) { return !seen.insert(p).second; }

     public:
      virtual void opfunc(base::OpBase *op) = 0;

      virtual void infunc(base::TerminalBase *in) = 0;

      virtual void outfunc(base::TerminalBase *out) = 0;
      ...
      static base::OpBase* to_OpBase_ptr(base::OpBase* op) { return op; }
      static base::OpBase* to_OpBase_ptr(const base::OpBase* op) {
        return const_cast<base::OpBase*>(op);
      }
  ...
  template <typename OpVisitor = detail::Traverse::null_visitor<base::OpBase>,
      typename InVisitor = detail::Traverse::null_visitor<base::TerminalBase>,
      typename OutVisitor = detail::Traverse::null_visitor<base::TerminalBase>>
  class Traverse : private detail::Traverse {
   public:
    static_assert(
        std::is_void<meta::void_t<decltype(std::declval<OpVisitor>()(std::declval<base::OpBase *>()))>>::value,
        "Traverse<OpVisitor,...>: OpVisitor(OpBase *op) must be a valid expression");
    static_assert(
        std::is_void<meta::void_t<decltype(std::declval<InVisitor>()(std::declval<base::TerminalBase *>()))>>::value,
        "Traverse<,InVisitor,>: InVisitor(TerminalBase *op) must be a valid expression");
    static_assert(
        std::is_void<meta::void_t<decltype(std::declval<OutVisitor>()(std::declval<base::TerminalBase *>()))>>::value,
        "Traverse<...,OutVisitor>: OutVisitor(TerminalBase *op) must be a valid expression");

    template <typename OpVisitor_ = detail::Traverse::null_visitor<base::OpBase>,
              typename InVisitor_ = detail::Traverse::null_visitor<base::TerminalBase>,
              typename OutVisitor_ = detail::Traverse::null_visitor<base::TerminalBase>>
    ...
    std::enable_if_t<(std::is_convertible_v<std::remove_reference_t<OpBasePtrs>,base::OpBase*> && ...),bool>
        operator()(base::OpBase* op, OpBasePtrs && ... ops) {
      reset();
   ...
   private:
    ...
    void opfunc(base::OpBase *op) { op_visitor_(op); }

    void infunc(base::TerminalBase *in) { in_visitor_(in); }

    void outfunc(base::TerminalBase *out) { out_visitor_(out); }
  };
