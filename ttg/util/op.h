...
namespace ttg {
  ...
  class CompositeOp : public base::OpBase {
   public:
   ...
   private:
    std::vector<std::unique_ptr<base::OpBase>> ops;
   ...
   public:
    ...
    CompositeOp(opsT &&ops_take_ownership,
        ...
        : base::OpBase(name, numins, numouts), ops(std::forward<opsT>(ops_take_ownership)), ins(ins), outs(outs) {
    ...
    base::OpBase *get_op(std::size_t i) { return ops.at(i).get(); }
  ...
  class OpSink : public base::OpBase {
  ...
  public:
    OpSink(const std::string& inname="junk") : base::OpBase("sink", numins, numouts) {
      register_input_terminals(input_terminals, std::vector<std::string>{inname});
      register_input_callback(std::get<0>(input_terminals));
    }

    OpSink(const input_edges_type &inedges, const std::string& inname="junk") : base::OpBase("sink", numins, numouts) {
      register_input_terminals(input_terminals, std::vector<std::string>{inname});
      register_input_callback(std::get<0>(input_terminals));
