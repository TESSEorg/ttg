...
namespace ttg {
  ...
  class In : public base::TerminalBase {
   public:
   ...
   private:
    ...
    void connect(base::TerminalBase *p) override {
   ...
   public:
    ...
    Type get_type() const override {
      return std::is_const<valueT>::value ? base::TerminalBase::Type::Read : base::TerminalBase::Type::Consume;
    }
  };

  // Output terminal
  template <typename keyT = void, typename valueT = void>
  class Out : public base::TerminalBase {
   public:
    typedef valueT value_type;
    typedef keyT key_type;
    static_assert(std::is_same<keyT, std::decay_t<keyT>>::value,
   ...
   public:
    ...
    void connect(base::TerminalBase *in) override {
#ifndef NDEBUG
      if (in->get_type() == base::TerminalBase::Type::Read) {
      ...
      } else if (in->get_type() == base::TerminalBase::Type::Consume) {
    ...
    std::enable_if_t<meta::is_none_void_v<Key,Value>,void> send(const Key &key, const Value &value) {
      for (auto && successor : successors()) {
        assert(successor->get_type() != base::TerminalBase::Type::Write);
        if (successor->get_type() == base::TerminalBase::Type::Read) {
    ...
    std::enable_if_t<!meta::is_void_v<Key> && meta::is_void_v<Value>,void> sendk(const Key &key) {
      for (auto && successor : successors()) {
        ...
        } else if (successor->get_type() == base::TerminalBase::Type::Consume) {
    ...
    send(const Key &key, Value &&value) {
      ...
      for (std::size_t i = 0; i != N; ++i) {
        if (successors().at(i)->get_type() == base::TerminalBase::Type::Consume) {
          move_terminal = i;
          break;
        }
      }
      if (N > 0) {
        ...
        for (std::size_t i = 0; i != N; ++i) {
          if (i != move_terminal) {
            base::TerminalBase *successor = successors().at(i);
            if (successor->get_type() == base::TerminalBase::Type::Read) {
              static_cast<In<keyT, std::add_const_t<valueT>> *>(successor)->send(key, value);
            } else if (successor->get_type() == base::TerminalBase::Type::Consume) {
              static_cast<In<keyT, valueT> *>(successor)->send(key, value);
        ...
        {
          base::TerminalBase *successor = successors().at(move_terminal);
    ...
    Type get_type() const override { return base::TerminalBase::Type::Write; }
  };
