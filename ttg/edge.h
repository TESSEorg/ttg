...
namespace ttg {
  ...
  class Edge {
   private:
    ...
    struct EdgeImpl {
      std::string name;
      std::vector<base::TerminalBase *> outs;  // In<keyT, valueT> or In<keyT, const valueT>
      ...
      void set_out(base::TerminalBase *out) {
      ...
      void try_to_connect_new_out(base::TerminalBase *out) const {
        assert(out->get_type() != base::TerminalBase::Type::Write);  // out must be an In<>
        for (auto in : ins)
          if (in && out) in->connect(out);
      }
   ...
   public:
    ...
    void set_out(base::TerminalBase *out) const {
