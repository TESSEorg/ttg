...
namespace ttg {
  namespace detail {
    Backtrace::Backtrace(const std::string &prefix) : prefix_(prefix) {
#ifdef TTG_HAS_LIBUNWIND
      {
        ...
        while (unw_step(&cursor) > 0) {
          ...
          oss << prefix_ << "frame " << frame << ": "
              << "ip = 0x" << (long)ip << " sp = 0x" << (long)sp << " symbol = " << __demangle(name);
      ...
      for (int i = 1; i < naddrs; ++i) {
...
#elif defined(__linux__)
        {
          ...
          const auto last_left_bracket = mangled_function_name.rfind('[', last_right_bracket);
          const auto last_right_parens = mangled_function_name.rfind(')', last_left_bracket);
          const auto offset = mangled_function_name.rfind("+0x", last_right_parens);
          const auto last_left_parens = mangled_function_name.rfind('(', last_right_parens);
          if (last_left_parens + 1 < mangled_function_name.size()) {
            mangled_function_name = mangled_function_name.substr(last_left_parens + 1, offset - last_left_parens - 1);
          }
        }
#endif

        std::ostringstream oss;
        oss << prefix_ << "frame " << i << ": return address = " << stack_addrs[i] << std::endl
      ...
      while (frame_pointer >= botstack && frame_pointer < topstack && frame_pointer[1] >= bottext &&
    ...
    std::string Backtrace::str(size_t nframes_to_skip) const {
      std::ostringstream oss;
      std::copy(frames_.begin() + nframes_to_skip, frames_.end(), std::ostream_iterator<std::string>(oss, "\n"));
      return oss.str();
    }
