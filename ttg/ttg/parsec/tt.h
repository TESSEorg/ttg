...
#include "ttg/parsec/comm_helper.h"
#include "ttg/parsec/hooks.h"
  ...
  class TT : public ttg::TTBase, detail::ParsecTTBase {
   ...
   private:
    ...
    void get_pull_terminal_data_from(const int owner,
      ...
      std::unique_ptr<msg_t> msg = std::make_unique<msg_t>(get_instance_id(), tp->taskpool_id,
                                                           detail::msg_header_t::MSG_GET_FROM_PULL, i, 1);
      /* pack the key */
      size_t pos = 0;
      ...
      parsec_ce.send_am(&parsec_ce, world_impl.parsec_ttg_tag(), owner, static_cast<void *>(msg.get()),
                        sizeof(detail::msg_header_t) + pos);
    }
   ...
   protected:
    ...
    static void static_set_arg(void *data, std::size_t size, ttg::TTBase *bop) {
      assert(size >= sizeof(detail::msg_header_t) &&
             "Trying to unpack as message that does not hold enough bytes to represent a single header");
      detail::msg_header_t *hd = static_cast<detail::msg_header_t *>(data);
      derivedT *obj = reinterpret_cast<derivedT *>(bop);
      switch (hd->fn_id) {
        case detail::msg_header_t::MSG_SET_ARG: {
          if (0 <= hd->param_id) {
        ...
        case detail::msg_header_t::MSG_SET_ARGSTREAM_SIZE: {
          assert(hd->param_id >= 0);
        ...
        case detail::msg_header_t::MSG_FINALIZE_ARGSTREAM_SIZE: {
          assert(hd->param_id >= 0);
        ...
        case detail::msg_header_t::MSG_GET_FROM_PULL: {
          assert(hd->param_id >= 0);
        ...
        if constexpr (!ttg::meta::is_void_v<valueT>) {
          ...
          if constexpr (!ttg::has_split_metadata<decvalueT>::value) {
            ...
            if (0 == num_iovecs) {
              ...
              assert(size == (pos + sizeof(detail::msg_header_t)));
            }
          }
          // case 2 and 3
    ...
    void set_arg_impl(const Key &key, Value &&value) {
      ...
      std::unique_ptr<msg_t> msg = std::make_unique<msg_t>(get_instance_id(), world_impl.taskpool()->taskpool_id,
                                                           detail::msg_header_t::MSG_SET_ARG, i, 1);
    ...
    broadcast_arg(const ttg::span<const Key> &keylist, const Value &value) {
      ...
      if (have_remote) {
        ...
        auto &world_impl = world.impl();
        std::unique_ptr<msg_t> msg = std::make_unique<msg_t>(get_instance_id(), world_impl.taskpool()->taskpool_id,
                                                             detail::msg_header_t::MSG_SET_ARG, i);
   ...
   public:
    ...
    std::enable_if_t<!ttg::meta::is_void_v<Key>, void> set_argstream_size(const Key &key, std::size_t size) {
      ...
      if (owner != world.rank()) {
        ...
        uint64_t pos = 0;
        std::unique_ptr<msg_t> msg = std::make_unique<msg_t>(get_instance_id(), world_impl.taskpool()->taskpool_id,
                                                             detail::msg_header_t::MSG_SET_ARGSTREAM_SIZE, i, 1);
        /* pack the key */
        pos = pack(key, msg->bytes, pos);
        msg->tt_id.num_keys = 1;
    ...
    std::enable_if_t<!ttg::meta::is_void_v<Key>, void> finalize_argstream(const Key &key) {
      ...
      if (owner != world.rank()) {
        ...
        uint64_t pos = 0;
        std::unique_ptr<msg_t> msg = std::make_unique<msg_t>(get_instance_id(), world_impl.taskpool()->taskpool_id,
                                                             detail::msg_header_t::MSG_FINALIZE_ARGSTREAM_SIZE, i, 1);
        /* pack the key */
        pos = pack(key, msg->bytes, pos);
        msg->tt_id.num_keys = 1;
        parsec_taskpool_t *tp = world_impl.taskpool();
   ...
   private:
    ...
    static parsec_hook_return_t complete_task_and_release(parsec_execution_stream_t *es, parsec_task_t *t) {
      parsec_execution_stream_t *safe_es = detail::parsec_ttg_es;
      detail::parsec_ttg_es = es;
   ...
   public:
    ...
    void register_static_op_function(void) {
      ...
      detail::static_set_arg_fct_call_t call = std::make_pair(&TT::static_set_arg, this);
      auto &world_impl = world.impl();
      detail::static_map_mutex.lock();
      detail::static_id_to_op_map.insert(std::make_pair(get_instance_id(), call));
      if (detail::delayed_unpack_actions.count(get_instance_id()) > 0) {
        auto tp = world_impl.taskpool();

        ttg::trace("ttg_parsec(", rank, ") There are ", detail::delayed_unpack_actions.count(get_instance_id()),
                   " messages delayed with op_id ", get_instance_id());

        auto se = detail::delayed_unpack_actions.equal_range(get_instance_id());
        std::vector<detail::static_set_arg_fct_arg_t> tmp;
        for (auto it = se.first; it != se.second;) {
          assert(it->first == get_instance_id());
          tmp.push_back(it->second);
          it = detail::delayed_unpack_actions.erase(it);
        }
        detail::static_map_mutex.unlock();
