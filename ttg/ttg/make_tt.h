...
//
template <typename funcT, typename keyT, typename output_terminalsT, typename PolicyT, typename... input_valuesT>
class CallableWrapTT : public TT<keyT, output_terminalsT,
                                 CallableWrapTT<funcT, keyT, output_terminalsT, PolicyT, input_valuesT...>,
 ...
 public:
  ...
  CallableWrapTT(funcT_ &&f, const input_edges_type &inedges, const output_edges_type &outedges,
      ...
      : baseT(inedges, outedges, name, innames, outnames, std::forward<PolicyT>(policy)),
...
template <typename funcT, typename keyT, typename output_terminalsT, typename input_values_tupleT, typename PolicyT>
struct CallableWrapTTUnwrapTuple;

template <typename funcT, typename keyT, typename output_terminalsT, typename... input_valuesT, typename PolicyT>
struct CallableWrapTTUnwrapTuple<funcT, keyT, output_terminalsT, std::tuple<input_valuesT...>, PolicyT> {
  using type = CallableWrapTT<funcT, keyT, output_terminalsT, PolicyT, std::remove_reference_t<input_valuesT>...>;
};
...
class CallableWrapTTArgs
    : public TT<keyT, output_terminalsT, CallableWrapTTArgs<funcT, keyT, output_terminalsT, PolicyT, input_valuesT...>,
                ttg::typelist<input_valuesT...>, PolicyT> {
...
template <typename funcT, typename keyT, typename output_terminalsT, typename PolicyT, typename input_values_tupleT>
struct CallableWrapTTArgsUnwrapTuple;

template <typename funcT, typename keyT, typename output_terminalsT, typename PolicyT, typename... input_valuesT>
struct CallableWrapTTArgsUnwrapTuple<funcT, keyT, output_terminalsT, PolicyT, std::tuple<input_valuesT...>> {
  using type = CallableWrapTTArgs<funcT, keyT, output_terminalsT, PolicyT, std::remove_reference_t<input_valuesT>...>;
};
...
auto make_tt_tpl(funcT &&func, const std::tuple<ttg::Edge<keyT, input_valuesT>...> &inedges,
  ...
  using wrapT = typename CallableWrapTTUnwrapTuple<funcT, keyT, output_terminals_type, input_args_t, PolicyT>::type;
...
auto make_tt(funcT &&func, const std::tuple<ttg::Edge<keyT, input_edge_valuesT>...> &inedges,
  ...
  using wrapT = typename CallableWrapTTArgsUnwrapTuple<funcT, keyT, output_terminals_type, PolicyT, full_input_args_t>::type;
  // not sure if we need this level of type checking ...
  // TODO determine the generic signature of func
  if constexpr (!void_key) {
    static_assert(
