...
class Flow_From_SpMatrix : public Op<ControlFlow, SpMatrixFlow, Flow_From_SpMatrix> {
 public:
  ...
  Flow_From_SpMatrix(const SpMatrix& matrix, const SpMatrixFlow& flow, const ControlFlow& ctl)
      : baseT(ctl, flow, "spmatrix_to_flow"), matrix_(matrix) {}
...
class Flow_To_SpMatrix : public Op<SpMatrixFlow, ControlFlow, Flow_To_SpMatrix> {
 public:
  ...
  Flow_To_SpMatrix(SpMatrix& matrix, const SpMatrixFlow& flow, const ControlFlow& ctl)
      : baseT(flow, ctl, "flow_to_spmatrix"), matrix_(matrix) {}
  void op(const Key<2>& key, const blk_t& elem, ControlFlow) { matrix_.insert(key[0], key[1]) = elem; }
...
class SpMM {
 public:
  SpMM(SpMatrixFlow a, SpMatrixFlow b, SpMatrixFlow c, const SpMatrix& a_mat, const SpMatrix& b_mat)
      : a_(a)
      , b_(b)
      , c_(c)
      , a_ijk_()
  ...
  class MultiplyAdd : public Op<FlowArray<Key<3>, blk_t, blk_t, blk_t>, SpMatrixFlow, MultiplyAdd> {
   public:
    ...
    MultiplyAdd(FlowArray<Key<3>, blk_t> a_ijk, FlowArray<Key<3>, blk_t> b_ijk, FlowArray<Key<3>, blk_t> c_ijk,
                SpMatrixFlow c, const std::vector<std::vector<long>>& a_rowidx_to_colidx,
    ...
    void op(const Key<3>& key, const blk_t& a_ijk, const blk_t& b_ijk, const blk_t& c_ijk, SpMatrixFlow result) {
      const auto i = key[0];
      const auto j = key[1];
      const auto k = key[2];
      long next_k;
