...
class Op : public ::ttg::OpBase, ParsecBaseOp {
 ...
 private:
  ...
  void set_arg(const keyT &key, T &&value) {
    ...
    task->user_tuple =
        std::align(alignment_of_input_tuple, sizeof(input_values_tuple_type), task_body_tail_ptr, task_body_tail_size);
 ...
 public:
  Op(const std::string &name, const std::vector<std::string> &innames,
    ...
    *((parsec_flow_t **) &(self.out[i])) = NULL;
    ...
    parsec_mempool_construct(&mempools,
                             OBJ_CLASS(parsec_task_t),
