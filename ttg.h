...
class TerminalBase {
 ...
 protected:
  ...
  const std::vector<TerminalBase *>& successors() const {
    return successors_;
  }
 ...
 protected:
  ...
  void register_terminal(terminalT &term, const std::string &name, const setfuncT setfunc) {
    term.set(this, i, name, detail::demangled_type_name<typename terminalT::key_type>(),
             ...
             out ? TerminalBase::Type::Write : (std::is_const<typename terminalT::value_type>::value ? TerminalBase::Type::Read : TerminalBase::Type::Consume)
    );
    (this->*setfunc)(i, &term);
  }
  ...
  void register_terminals(std::index_sequence<IS...>, terminalsT &terms, const namesT &names,
    ...
    int junk[] = {0, (register_terminal<out,typename std::tuple_element<IS, terminalsT>::type, IS>(std::get<IS>(terms),
                                                                                               names[IS], setfunc),
        0)...};
    junk[0]++;
  }
...
class In : public TerminalBase {
 public:
  ...
  static_assert(std::is_same<keyT,std::decay_t<keyT>>::value, "In<keyT,valueT> assumes keyT is a non-decayable type");
...
class Out : public TerminalBase {
 public:
  typedef valueT value_type;
  typedef keyT key_type;
  static_assert(std::is_same<keyT,std::decay_t<keyT>>::value, "Out<keyT,valueT> assumes keyT is a non-decayable type");
  static_assert(std::is_same<valueT,std::decay_t<valueT>>::value, "Out<keyT,valueT> assumes valueT is a non-decayable type");
  typedef Edge<keyT, valueT> edge_type;
  ...
  void connect(TerminalBase *in) {
    ...
    if (in->get_type() == TerminalBase::Type::Read) {
      ...
      if (!dynamic_cast<input_terminal_type *>(in))
        throw std::invalid_argument(std::string("you are trying to connect terminals with incompatible types:\ntype of this Terminal = ")
...
class Edge {
 ...
 public:
  ...
  typedef keyT key_type;
  typedef valueT value_type;
  static_assert(std::is_same<keyT,std::decay_t<keyT>>::value, "Edge<keyT,valueT> assumes keyT is a non-decayable type");
  static_assert(std::is_same<valueT,std::decay_t<valueT>>::value, "Edge<keyT,valueT> assumes valueT is a non-decayable type");
