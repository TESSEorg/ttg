...
static parsec_hook_return_t hook(struct parsec_execution_unit_s* eu, parsec_execution_context_t* task);
}
 ...
 private:
  ...
  void set_arg(const keyT& key, const valueT& value) {
    ...
    if (args.counter == 0) {
      ...
      task->function_template_class_ptr = reinterpret_cast<void (*)(void*)>(&Op::static_op);
 ...
 public:
  ...
  static void static_op(parsec_execution_context_t* my_task) {
    ...
    */
    ...
    ((derivedT*)task->object_ptr)->op((keyT)task->key, *reinterpret_cast<input_valuesT*>(task->data[0].data_in));
#ifdef OR_BETTER
...
class Merge : public Op<keyT, std::tuple<valueT, valueT>, std::tuple<OutEdge<keyT, valueT>>, Merge<keyT, valueT>> {
 ...
 public:
  ...
  InEdge<keyT, valueT> in() {
    ...
    return edgeT(this->self.in[i], this->self.function_id, &baseT::template set_arg_static<i, valueT>);
  }
  // Returns the output edge i to facilitate connection
  template <std::size_t i>
