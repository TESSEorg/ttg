...
class Flow {
 ...
 public:
  ...
  void add_callback(const callbackT& callback) const { p->callbacks.push_back(callback); }
...
Flow<keyT, valueT> clone(const Flow<keyT, valueT>& inflow) {
  ...
  inflow.add_callback(
      [outflow](const keyT& key, const valueT& value) { const_cast<Flow<keyT, valueT>*>(&outflow)->send(key, value); });
  return outflow;
}
...
namespace detail {
  ...
  struct key_type<flowT, flowTs...> : public key_type<flowTs...> {
    ...
    typedef typename std::conditional<std::is_same<typename key_type<flowT>::type, typename baseT::type>::value,
  ...
  struct flows_typedefs
      : public std::conditional<std::is_same<typename key_type<flowTs...>::type, invalid_key_type>::value,
                                dont_define_key_typedefs,
                                do_define_key_typedefs<typename key_type<flowTs...>::type,
                                                       typename detail::value_type<flowTs>::type...>>::type {
    typedef std::tuple<typename detail::value_type<flowTs>::type...> values_tuple_type;
  };
...
auto make_flows(flowsT... args) {
  return Flows<flowsT...>(args...);
}
...
class OpTuple {
 ...
 private:
  ...
  struct OpTupleImpl : private BaseOp {
    ...
    void register_input_callback(const flowT& input) {
      ...
      auto callback = [this](const input_key_type& key, const typename flowT::value_type& value) {
        set_arg<typename flowT::value_type, i>(key, value);
      };
    ...
    void register_input_callbacks(const Tuple& inputs, std::index_sequence<IS...>) {
      int junk[] = {
          0, (register_input_callback<typename std::tuple_element<IS, Tuple>::type, IS>(std::get<IS>(inputs)), 0)...};
      junk[0]++;
    }
 ...
 public:
  ...
  void connect(const input_flowsT& inputs, const output_flowsT& outputs) {
    ...
    impl->register_input_callbacks(
        inputs.all(), std::make_index_sequence<std::tuple_size<typename input_flowsT::values_tuple_type>::value>{});
  }
...
class Op : public OpTuple<input_flowsT, output_flowsT, Op<input_flowsT, output_flowsT, derivedT>> {
 public:
  ...
  void call_op_from_tuple(const input_key_type& key, const input_values_tuple_type& params, output_type& output,
                          std::index_sequence<S...>) {
    static_cast<derivedT*>(this)->op(key, std::get<S>(params)..., output);
  }

 public:
  Op(const std::string& name = std::string("unamed arg op")) : opT(name) {}
...
class WrapOpTuple : public OpTuple<input_flowsT, output_flowsT, WrapOpTuple<funcT, input_flowsT, output_flowsT>> {
 ...
 public:
  WrapOpTuple(const funcT& func, const input_flowsT& inflows, const output_flowsT& outflows,
              const std::string& name = "wrapper")
      : baseT(inflows, outflows, name), func(func) {}

  void op(const typename baseT::input_key_type& key, const typename baseT::input_values_tuple_type& args,
          output_flowsT& out) {
    func(key, args, out);
  }
};
...
template <typename funcT, typename input_flowsT, typename output_flowsT>
auto make_optuple_wrapper(const funcT& func, const input_flowsT& inputs, const output_flowsT& outputs,
                          const std::string& name = "wrapper") {
...
class WrapOp : public Op<input_flowsT, output_flowsT, WrapOp<funcT, input_flowsT, output_flowsT>> {
 ...
 public:
  WrapOp(const funcT& func, const input_flowsT& inflows, const output_flowsT& outflows,
         const std::string& name = "wrapper")
      : baseT(inflows, outflows, name), func(func) {}
  ...
  void op(const input_keyT& key, const value0T& value0, const value1T& value1, const value2T& value2,
          output_flowsT& out) {
...
template <typename funcT, typename input_flowsT, typename output_flowsT>
auto make_op_wrapper(const funcT& func, const input_flowsT& inputs, const output_flowsT& outputs,
                     const std::string& name = "wrapper") {
