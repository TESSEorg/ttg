...
auto make_array_crude(const Ts&...t) {return std::array<T,N>{{t...}};}
...
template <typename T> const T& value(const T& t) {return t;}
template <typename T> const T& value(const T* t) {return*t;}
template <typename T> const T& value(      T* t) {return*t;}
