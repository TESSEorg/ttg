                     ...
                     ttg::Edge<Key2, MatrixTile<typename MatrixT::element_type>>& to_trsml_kn)
{
  ...
  auto f = [=](const Key2& key,
    ...
    if(key[0] == key[1]) {
      ...
      if(key[0] < A.rows()) {
          keylist_trsml.reserve(A.rows()-key[0]+1);
      ...
      ttg::broadcast<0, 1, 2>(std::make_tuple(Key1{key[0]}, std::move(keylist_trsmr), std::move(keylist_trsml)), std::move(tile), out);
      return;
    }

    if(ttg::tracing()) ttg::print("TRTRI_Dispatch(", key, ") sending to input_kn of TRSM_L(", Key2{key[0], key[1]}, ")");
    ttg::send<3>(Key2{key[0], key[1]}, std::move(tile), out);
  };
