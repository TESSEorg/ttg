...
struct Key {
  ...
  auto hash() const { return hash_val; }
  void rehash() {
    std::hash<int> int_hasher;
    hash_val = (int_hasher(I) << 48) ^ (int_hasher(J) << 32) ^ (K << 16);
  }
...
void plgsy(Matrix<double>* A) {
  ...
  for (int i = 0; i < A->rows(); ++i) {
    for (int j = 0; j < A->cols(); ++j) {
      auto tile = (*A)(i, j);
      CORE_dplgsy(bump, tile.rows(), tile.cols(), tile.get(), tile.rows(), A->rows(), i * tile.rows(), j * tile.cols(),
                  seed);
    }
  }
}
...
template <typename T>
auto make_potrf(Matrix<T>* A, ttg::Edge<Key, BlockMatrix<T>>& input, ttg::Edge<Key, BlockMatrix<T>>& output_trsm,
                ttg::Edge<Key, BlockMatrix<T>>& output_result) {
  auto f = [=](const Key& key, BlockMatrix<T>&& tile_kk,
               std::tuple<ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>>& out) {
    const int I = key.I;
    const int J = key.J;
    const int K = key.K;
    assert(I == J);
  ...
  return ttg::wrap(f, ttg::edges(input), ttg::edges(output_result, output_trsm), "POTRF", {"tile_kk"},
                   {"output_result", "output_trsm"});
}

template <typename T>
auto make_trsm(Matrix<T>* A, ttg::Edge<Key, BlockMatrix<T>>& input_kk, ttg::Edge<Key, BlockMatrix<T>>& input_mk,
               ttg::Edge<Key, BlockMatrix<T>>& output_diag, ttg::Edge<Key, BlockMatrix<T>>& output_row,
               ttg::Edge<Key, BlockMatrix<T>>& output_col, ttg::Edge<Key, BlockMatrix<T>>& output_result) {
  auto f = [=](const Key& key, const BlockMatrix<T>& tile_kk, BlockMatrix<T>&& tile_mk,
               std::tuple<ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>,
    ...
    blas::trsm(blas::Layout::RowMajor, blas::Side::Right, lapack::Uplo::Lower, blas::Op::Trans, blas::Diag::NonUnit,
               tile_kk.rows(), m, 1.0, tile_kk.get(), m, tile_mk.get(), m);
...
template <typename T>
auto make_syrk(Matrix<T>* A, ttg::Edge<Key, BlockMatrix<T>>& input_mk, ttg::Edge<Key, BlockMatrix<T>>& input_mm,
               ttg::Edge<Key, BlockMatrix<T>>& output_potrf, ttg::Edge<Key, BlockMatrix<T>>& output_syrk) {
  auto f = [=](const Key& key, const BlockMatrix<T>& tile_mk, BlockMatrix<T>&& tile_mm,
               std::tuple<ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>>& out) {
    const int I = key.I;
    const int J = key.J;
    const int K = key.K;
    ...
    blas::syrk(blas::Layout::RowMajor, lapack::Uplo::Lower, blas::Op::NoTrans, tile_mk.rows(), m, -1.0, tile_mk.get(),
               m, 1.0, tile_mm.get(), m);
  ...
  return ttg::wrap(f, ttg::edges(input_mk, input_mm), ttg::edges(output_potrf, output_syrk), "SYRK",
                   {"tile_mk", "tile_mm"}, {"output_potrf", "output_syrk"});
}

template <typename T>
auto make_gemm(Matrix<T>* A, ttg::Edge<Key, BlockMatrix<T>>& input_nk, ttg::Edge<Key, BlockMatrix<T>>& input_mk,
               ttg::Edge<Key, BlockMatrix<T>>& input_nm, ttg::Edge<Key, BlockMatrix<T>>& output_trsm,
               ttg::Edge<Key, BlockMatrix<T>>& output_gemm) {
  auto f = [](const Key& key, const BlockMatrix<T>& tile_nk, const BlockMatrix<T>& tile_mk, BlockMatrix<T>&& tile_nm,
              std::tuple<ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>>& out) {
    const int I = key.I;
    const int J = key.J;
    const int K = key.K;
    ...
    blas::gemm(blas::Layout::RowMajor, blas::Op::NoTrans, blas::Op::Trans, m, m, m, -1.0, tile_nk.get(), m,
               tile_mk.get(), m, 1.0, tile_nm.get(), m);
  ...
  return ttg::wrap(f, ttg::edges(input_nk, input_mk, input_nm), ttg::edges(output_trsm, output_gemm), "GEMM",
                   {"input_nk", "input_mk", "input_nm"}, {"output_trsm", "outout_gemm"});
}

template <typename T>
auto initiator(Matrix<T>* A, ttg::Edge<Key, BlockMatrix<T>>& syrk_potrf, ttg::Edge<Key, BlockMatrix<T>>& gemm_trsm,
               ttg::Edge<Key, BlockMatrix<T>>& syrk_syrk, ttg::Edge<Key, BlockMatrix<T>>& gemm_gemm) {
  auto f = [=](const Key&, std::tuple<ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>,
                                      ttg::Out<Key, BlockMatrix<T>>, ttg::Out<Key, BlockMatrix<T>>>& out) {
...
int main(int argc, char** argv) {
  ...
  ttg::Edge<Key, BlockMatrix<double>> potrf_trsm("potrf_trsm"), trsm_syrk("trsm_syrk"), syrk_potrf("syrk_potrf"),
      syrk_syrk("syrk_syrk"), gemm_gemm("gemm_gemm"), gemm_trsm("gemm_trsm"), trsm_gemm_row("trsm_gemm_row"),
      trsm_gemm_col("trsm_gemm_col"), result("result");
  ...
  auto op_trsm = make_trsm(A, potrf_trsm, gemm_trsm, trsm_syrk, trsm_gemm_row, trsm_gemm_col, result);
  auto op_syrk = make_syrk(A, trsm_syrk, syrk_syrk, syrk_potrf, syrk_syrk);
  auto op_gemm = make_gemm(A, trsm_gemm_row, trsm_gemm_col, gemm_gemm, gemm_trsm, gemm_gemm);
