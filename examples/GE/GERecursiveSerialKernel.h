...
void ge_recursive_serial_kernelA(double* X, int problem_size,
								 int block_size, int i_lb, int j_lb, int k_lb,
								 int recurisve_fan_out, int base_size);

// X = V, U = W
void ge_recursive_serial_kernelB(double* X, double* U, int problem_size,
								 int block_size, int i_lb, int j_lb, int k_lb,
								 int recurisve_fan_out, int base_size);

// X = U, V = W
void ge_recursive_serial_kernelC(double* X, double* V, int problem_size,
								 int block_size, int i_lb, int j_lb, int k_lb,
								 int recurisve_fan_out, int base_size);

// Everything is disjoint
void ge_recursive_serial_kernelD(double* X, double* U, double* V, double* W,
...
void ge_recursive_serial_kernelA(double* X, int problem_size,
	...
	if(block_size <= base_size || block_size <= recurisve_fan_out) {
		...
		for(int kk = 0; kk < recurisve_fan_out; ++kk) {
			ge_recursive_serial_kernelA(X, problem_size, tile_size,
										  i_lb + kk * tile_size,
										  j_lb + kk * tile_size,
										  k_lb + kk * tile_size,
			...
			for(int ii = kk+1; ii < recurisve_fan_out; ++ii) {
				// #pragma omp task
				ge_recursive_serial_kernelB(X, X, problem_size, tile_size,
											i_lb + kk * tile_size,
											j_lb + ii * tile_size,
											k_lb + kk * tile_size,
											recurisve_fan_out, base_size);
				// #pragma omp task
				ge_recursive_serial_kernelC(X, X, problem_size, tile_size,
											i_lb + ii * tile_size,
											j_lb + kk * tile_size,
											k_lb + kk * tile_size,
											recurisve_fan_out, base_size);
			...
			for(int ii = kk+1; ii < recurisve_fan_out; ++ii) {
				for(int jj = kk+1; jj < recurisve_fan_out; ++jj) {
					// #pragma omp task
					ge_recursive_serial_kernelD(X, X, X, X, problem_size,
												tile_size,
												i_lb + ii * tile_size,
											   	j_lb + jj * tile_size,
											    k_lb + kk * tile_size,
	...
	if(block_size <= base_size || block_size <= recurisve_fan_out) {
		...
		for(int kk = 0; kk < recurisve_fan_out; ++kk) {
			for(int jj = 0; jj < recurisve_fan_out; ++jj) {
				// #pragma omp task
				ge_recursive_serial_kernelB(X, U, problem_size,
											tile_size,
											i_lb + kk * tile_size,
											j_lb + jj * tile_size,
											k_lb + kk * tile_size,
			...
			for(int ii = kk+1; ii < recurisve_fan_out; ++ii) {
				for(int jj = 0; jj < recurisve_fan_out; ++jj) {
					// #pragma omp task
					ge_recursive_serial_kernelD(X, U, X, U, problem_size, tile_size,
											   	i_lb + ii * tile_size,
											   	j_lb + jj * tile_size,
											    k_lb + kk * tile_size,
											    recurisve_fan_out, base_size);
	...
	if(block_size <= base_size || block_size <= recurisve_fan_out) {
		...
		for(int kk = 0; kk < recurisve_fan_out; ++kk) {
			for(int ii = 0; ii < recurisve_fan_out; ++ii) {
				// #pragma omp task
				ge_recursive_serial_kernelC(X, V, problem_size,
											tile_size,
											i_lb + ii * tile_size,
											j_lb + kk * tile_size,
											k_lb + kk * tile_size,
			...
			for(int jj = kk+1; jj < recurisve_fan_out; ++jj) {
				for(int ii = 0; ii < recurisve_fan_out; ++ii) {
					// #pragma omp task
					ge_recursive_serial_kernelD(X, X, V, V, problem_size, tile_size,
											   	i_lb + ii * tile_size,
											   	j_lb + jj * tile_size,
											    k_lb + kk * tile_size,
											    recurisve_fan_out, base_size);
	...
	if(block_size <= base_size || block_size <= recurisve_fan_out) {
		...
		for(int kk = 0; kk < recurisve_fan_out; ++kk) {
			for(int ii = 0; ii < recurisve_fan_out; ++ii) {
				for(int jj = 0; jj < recurisve_fan_out; ++jj) {
					// #pragma omp task
					ge_recursive_serial_kernelD(X, U, V, W, problem_size, tile_size,
												   i_lb + ii * tile_size,
												   j_lb + jj * tile_size,
												   k_lb + kk * tile_size,
												   recurisve_fan_out, base_size);
