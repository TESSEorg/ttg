...
//
// eri-pch v6 -- v5 + significant-shell-pair screening.
//
// v1..v5 build the AO-pair layout from the *full* upper-triangular shell-
// pair list (s1, s2 >= s1). For an extended molecule that scales as
// O(N_shell^2) and the matrix M = (mu nu | alpha beta) blows up with the
// system size even though the physically non-negligible block of M is
// O(N_shell). v6 restricts the layout to the Almlöf-significant shell
// pairs (those whose overlap-matrix block has Frobenius norm above the
// screening threshold; same-center pairs always kept), the same notion
// libint2's hartree-fock++ test uses for `compute_shellpairs`. The TTG
// graph is otherwise identical to v5.
//
// The threshold defaults to 1e-12 (matching libint2 hartree-fock++) and
// can be overridden via the env var TTG_ERI_PCH_SP_THRESHOLD; setting
// it to 0 disables screening and reproduces v5.
//
...
//
// v5/v6 makes the data part of the flow:
//
...
//
// Usage: eri-pch_v6-{mad,parsec} [<molecule.xyz> [<basis-name> [<tolerance>]]]
//
// Side effect: writes the TTG flow graph to `eri-pch_v6.dot`.
...
Eigen::MatrixXd run_cholesky(CholeskyDriver& drv, double tol,
  ...
  if (dump_dot_on_root && ttg::default_execution_context().rank() == 0) {
    ...
    std::ofstream out("eri-pch_v6.dot");
    out << graph_str;
    std::cout << "[eri-pch] wrote TTG graph to eri-pch_v6.dot" << std::endl;
  }
...
int main(int argc, char* argv[]) {
  ...
  // Default screening threshold mirrors libint2's hartree-fock++ test
  // (`compute_shellpairs` calls it with 1e-12). Override via env var; 0
  // disables screening and reproduces v5.
  double sp_threshold = 1e-12;
  if (const char* env = std::getenv("TTG_ERI_PCH_SP_THRESHOLD")) {
    sp_threshold = std::atof(env);
  }

  eri_pch::IntegralData D;
  D.build(std::move(atoms), args.basis_name, sp_threshold);

  const bool root = (ttg::default_execution_context().rank() == 0);
  if (root) {
    std::cout << "[eri-pch] shell-pair screening: threshold = "
              << std::scientific << std::setprecision(2) << sp_threshold
              << std::defaultfloat
              << "; kept " << D.nshell_pairs() << " of "
              << (D.nshell * (D.nshell + 1) / 2)
              << " shell pairs ("
              << D.npairs() << " AO pairs)" << std::endl;
  }
