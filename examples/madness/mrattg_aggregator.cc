...
void do_compress(const Key<NDIM>& key,
                 const ttg::Aggregator<FunctionReconstructedNodeWrap<T,K,NDIM>> &in,
                 std::tuple<rnodeOut<T,K,NDIM>, cnodeOut<T,K,NDIM>> &out) {
  ...
  //typename ::detail::tree_types<T,K,NDIM>::compress_out_type& out) {
    ...
    {   // Collect child coeffs and leaf info
        ...
        int i = 0;
        for (auto&& c : in) {
          s(child_slices[i]) = c.get().coeffs;
          result.get().is_leaf[i] = c.get().is_leaf;
          sumsq += c.get().sum; // Accumulate sumsq from child difference coeffs
          i++;
        }
...
auto make_compress(rnodeEdge<T,K,NDIM>& in, cnodeEdge<T,K,NDIM>& out, const std::string& name = "compress") {
  rnodeEdge<T,K,NDIM> children("children");

  return std::make_tuple(ttg::make_tt(&send_leaves_up<T,K,NDIM>, ttg::edges(in), ttg::edges(children, out), "send_leaves_up", {"input"}, {"children", "output"}),
                         ttg::make_tt(&do_compress<T,K,NDIM>, ttg::edges(ttg::make_aggregator(children, 1 << NDIM)), ttg::edges(children,out), "do_compress", {"children"}, {"recur","output"}));
}
