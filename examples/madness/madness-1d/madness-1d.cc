...
double gaussian(double x, double a, double coeff) { return coeff * exp(-a * x * x); }

double funcD(double x) { return gaussian((x - 0.5), 100.0, 1.0); }

double funcDD(double x) { return -200 * (x - 0.5) * gaussian((x - 0.5), 100.0, 1.0); }
...
class Node {
 public:
  ...
  Node(const Key &input_key, const Vector &s_input, const Vector &d_input, bool has_children_input)
      : key(input_key), s(s_input), d(d_input), has_children(has_children_input) {}
...
class GaxpyOp : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, GaxpyOp, Node, Node> {
 ...
 public:
  ...
  GaxpyOp(const double &alpha, const double &beta, const typename baseT::input_edges_type &inedges,
          const typename baseT::output_edges_type &outedges, const std::string &name)
      : baseT(inedges, outedges, name, {"input_op1", "input_op2"}, {"iterator_op1", "result", "iterator_op2"})
      , alpha(alpha)
      , beta(beta) {}
...
class BinaryOp : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, BinaryOp, Node, Node> {
 ...
 public:
  BinaryOp(const funcT &func, const std::string &name)
      : baseT(name, {"input_a", "input_b"}, {"iterator_a", "result", "iterator_b"}), func(func) {}

  BinaryOp(const funcT &func, const typename baseT::input_edges_type &inedges,
           const typename baseT::output_edges_type &outedges, const std::string &name)
      : baseT(inedges, outedges, name, {"input_a", "input_b"}, {"iterator_a", "result", "iterator_b"}), func(func) {}
  ...
  void op(const Key &key, const std::tuple<Node, Node> &t, baseT::output_terminals_type &out) {
    ...
    if (left.s.length() && right.s.length()) {
      ...
      if (left.s.length()) {
        ...
        ::send<0>(key.right_child(), Node(key.right_child(), left_unfiltered.get_slice(k, 2 * k), Vector(), false),
                  out);
      }
      if (right.s.length()) {
        ...
        ::send<2>(key.right_child(), Node(key.right_child(), right_unfiltered.get_slice(k, 2 * k), Vector(), false),
                  out);
      }
...
class Diff_prologue : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, Diff_prologue, Node> {
 ...
 public:
  Diff_prologue(const std::string &name) : baseT(name, {"input"}, {"L", "C", "R"}) {}

  Diff_prologue(const typename baseT::input_edges_type &inedges, const typename baseT::output_edges_type &outedges,
...
class Diff_doIt : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, Diff_doIt,
                            Node, Node, Node> {
  using baseT =
      Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, Diff_doIt, Node, Node, Node>;
 ...
 public:
  ...
  Diff_doIt(const typename baseT::input_edges_type &inedges, const typename baseT::output_edges_type &outedges,
            const std::string &name)
      : baseT(inedges, outedges, name, {"L_input", "C_input", "R_input"},
              {"L_output", "C_output", "R_output", "output"}) {}
  ...
  void op(const Key &key, const std::tuple<Node, Node, Node> &t, baseT::output_terminals_type &out) {
    ...
    if (left.s.length() != 0 && center.s.length() != 0 && right.s.length() != 0) {
      // std::cout << "left.s.length() is " << left.s.length() << ", center.s.length() is " << center.s.length() << "
      // and right.s.length() is " << right.s.length() << std::endl;
...
class Compress_prologue
    : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, Compress_prologue, Node> {
 ...
 public:
  ...
  Compress_prologue(const typename baseT::input_edges_type &inedges, const typename baseT::output_edges_type &outedges,
                    const std::string &name)
...
class Compress_doIt
    : public Op<Key, std::tuple<Out<Key, Node>, Out<Key, Node>, Out<Key, Node>>, Compress_doIt, Node, Node> {
 ...
 public:
  ...
  Compress_doIt(const typename baseT::input_edges_type &inedges, const typename baseT::output_edges_type &outedges,
                const std::string &name)
...
class Reconstruct_prologue : public Op<Key, std::tuple<Out<Key, Vector>>, Reconstruct_prologue, Node> {
 ...
 public:
  Reconstruct_prologue(const std::string &name) : baseT(name, {"input_node"}, {"output_vector"}) {}
...
class Reconstruct_doIt : public Op<Key, std::tuple<Out<Key, Vector>, Out<Key, Node>>, Reconstruct_doIt, Vector, Node> {
 ...
 public:
  ...
  Reconstruct_doIt(const typename baseT::input_edges_type &inedges, const typename baseT::output_edges_type &outedges,
                   const std::string &name)
...
class Everything : public Op<Key, std::tuple<>, Everything> {
 ...
 public:
  ...
  void start() {
    if (world.rank() == 0) producer.invoke(Key(0, 0));
  }
