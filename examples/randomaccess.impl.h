...
class RandomData {
 private:
  ...
  std::shared_ptr<unsigned long> data;
...
auto make_randomgen_op(params_t &tparams, 
		     ...
		     std::tuple<Out<Key, unsigned long>, Out<Key, RandomData>, Out<Key, RandomData>> &out)
  {
    ...
    if (iter < niterate) {
      ...
      //std::cout << "GlobalStartMyProc : " << tparams.GlobalStartMyProc << ", ran : " << ran << ", logTableLocal : " << logTableLocal << ", nlocalm1 : " << nlocalm1 << std::endl;
...
void
Power2NodesMPIRandomAccessUpdateVerfy(std::vector<unsigned long> verify_table, params_t tparams) {
  int i, j, logTableLocal,ipartner;
  int ndata, nkeep, nsend, nrecv;
  ...
  for (iterate = 0; iterate < niterate; iterate++) {
    ...
    for (j = 0; j < tparams.logNumProcs; j++) {
      ...
      MPI_Sendrecv(send,nsend,MPI_UNSIGNED_LONG,ipartner,0,
                   &data[nkeep],CHUNKBIG, MPI_UNSIGNED_LONG,
