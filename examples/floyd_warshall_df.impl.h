...
class Initiator : public Op<int,
							...
							Initiator<T>> {
	using baseT = Op<int, std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
						Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, Initiator>;
...
public:
	...
	Initiator(Matrix<T>* adjacency_matrix_ttg, const typename baseT::output_edges_type& outedges,
	...
	void op(const int& iterations, typename baseT::output_terminals_type& out) {
		...
		for(int i = 0; i < iterations; ++i) {
			for(int j = 0; j < iterations; ++j) {
    			...
    			::send<1>(Key(std::make_pair(std::make_pair(i, j), 0)), (*adjacency_matrix_ttg)(i,j), out);
				}
				else if(j == 0) { // C function call
					::send<2>(Key(std::make_pair(std::make_pair(i, j), 0)), (*adjacency_matrix_ttg)(i,j), out);
				}
...
public:
  Finalizer(Matrix<T>* result_matrix_ttg, int problem_size, int blocking_factor,
      const std::string& kernel_type, int recursive_fan_out, int base_size,
      const std::string& name)
  : baseT(name, {"input"}, {}),
  result_matrix_ttg(result_matrix_ttg), problem_size(problem_size),
  ...
  void op(const Key& key, typename baseT::input_values_tuple_type&& t,
...
class FuncA : public Op<Key,
	...
	using baseT = Op<Key, std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								     Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								     Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>,
							FuncA, BlockMatrix<T>>;
	Matrix<T>* adjacency_matrix_ttg;
...
public:
	FuncA(Matrix<T>* adjacency_matrix_ttg, int problem_size, int blocking_factor,
		  const std::string& kernel_type, int recursive_fan_out, int base_size,
		  const std::string& name)
	...
	void op(const Key& key, typename baseT::input_values_tuple_type &&t,//const std::tuple<BlockMatrix<T>>&& t,
		    typename baseT::output_terminals_type& out) {
		int I = key.execution_info.first.first;
		int J = key.execution_info.first.second;
    ...
    BlockMatrix<T> m_ij;
      ...
      m_ij = floyd_iterative_kernel(problem_size/blocking_factor, (get<0>(t)), (get<0>(t)), (get<0>(t)));
		...
		for(int l = 0; l < blocking_factor; ++l) {
			if (l != K) {
				// B calls
				//::send<4>(Key(std::make_pair(std::make_pair(I, l), K)), (*adjacency_matrix_ttg)(I,l), out);
				// C calls
				//::send<5>(Key(std::make_pair(std::make_pair(l, J), K)), (*adjacency_matrix_ttg)(l,J), out);
			  // B calls
...
class FuncB : public Op<Key,
						std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncB<T>,
							 	   BlockMatrix<T>, BlockMatrix<T>> {
	using baseT = Op<Key, std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncB,
							 	   BlockMatrix<T>, BlockMatrix<T>>;
	Matrix<T>* adjacency_matrix_ttg;
	int problem_size;
	int blocking_factor;
...
public:
	FuncB(Matrix<T>* adjacency_matrix_ttg, int problem_size, int blocking_factor,
		  const std::string& kernel_type, int recursive_fan_out, int base_size,
		  const std::string& name)
	...
	void op(const Key& key, typename baseT::input_values_tuple_type &&t, //const std::tuple<BlockMatrix<T>, BlockMatrix<T>>&& t,
		    typename baseT::output_terminals_type& out) {
		int I = key.execution_info.first.first;
		int J = key.execution_info.first.second;
		int K = key.execution_info.second;
...
class FuncC : public Op<Key,
						std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncC<T>,
							 	   BlockMatrix<T>, BlockMatrix<T>> {
	using baseT = Op<Key, std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncC,
							 	   BlockMatrix<T>, BlockMatrix<T>>;
	Matrix<T>* adjacency_matrix_ttg;
	int problem_size;
	int blocking_factor;
...
public:
	FuncC(Matrix<T>* adjacency_matrix_ttg, int problem_size, int blocking_factor,
		  const std::string& kernel_type, int recursive_fan_out, int base_size,
		  const std::string& name)
...
class FuncD : public Op<Key,
						std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncD<T>,
							 	   BlockMatrix<T>, BlockMatrix<T>, BlockMatrix<T>> 
{
	using baseT = Op<Key, std::tuple<Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>,
								   Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>, Out<Key, BlockMatrix<T>>>, FuncD,
							 	   BlockMatrix<T>, BlockMatrix<T>, BlockMatrix<T>>;
	Matrix<T>* adjacency_matrix_ttg;
	int problem_size;
	int blocking_factor;
...
public:
	FuncD(Matrix<T>* adjacency_matrix_ttg, int problem_size, int blocking_factor,
		  const std::string& kernel_type, int recursive_fan_out, int base_size,
		  const std::string& name)
	...
	void op(const Key& key, typename baseT::input_values_tuple_type &&t, //const std::tuple<BlockMatrix<T>, BlockMatrix<T>, BlockMatrix<T>>&& t,
		    typename baseT::output_terminals_type& out) {
		int I = key.execution_info.first.first;
		int J = key.execution_info.first.second;
		int K = key.execution_info.second;
...
void init_square_matrix(int problem_size, int blocking_factor,
						bool verify_results, double* adjacency_matrix_serial, Matrix<double>* m);
//Print the matrix
void print_square_matrix(int problem_size, int blocking_factor,
...
int main(int argc, char** argv) {
   ...
   FloydWarshall fw_apsp(m, r, problem_size, blocking_factor, kernel_type, 
   				 recursive_fan_out, base_size);
  ...
  //print_square_matrix(problem_size, blocking_factor, verify_results, adjacency_matrix_serial);
  cout << endl << endl << endl;
  //m->print();
  delete m;
  return 0;
