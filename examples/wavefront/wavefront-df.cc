  ...
  namespace archive {
    ...
    struct ArchiveStoreImpl<Archive, std::tuple<std::pair<T, T>, std::pair<T, T>>> {
      static inline void store(const Archive& ar, const std::tuple<std::pair<T, T>, std::pair<T, T>>& p) {
        std::cout << "Storing..." << std::get<0>(p).first << std::get<0>(p).second << std::endl;
        ar& std::get<0>(p).first& std::get<0>(p).second;
        ar& std::get<1>(p).first& std::get<1>(p).second;
        std::cout << "Stored..." << std::get<1>(p).first << std::get<1>(p).second << std::endl;
      }
    };
    ...
    struct ArchiveStoreImpl<Archive, std::unordered_map<std::pair<int, int>, BlockMatrix<T>>> {
      static inline void store(const Archive& ar, const std::unordered_map<std::pair<int, int>, BlockMatrix<T>>& m) {
    ...
    struct ArchiveLoadImpl<Archive, std::unordered_map<std::pair<int, int>, BlockMatrix<T>>> {
      static inline void load(const Archive& ar, std::unordered_map<std::pair<int, int>, BlockMatrix<T>>& m) {
...
auto initiator(int MB, int NB, std::unordered_map<Key, BlockMatrix<T>, pair_hash>& m,
  ...
  auto f = [&m, MB, NB](const Key& key,
    ...
    send<0>(Key(0, 0), m.at(std::make_pair(0, 0)), out);
  };
...
auto make_get_inputblock(std::unordered_map<Key, BlockMatrix<T>, pair_hash>& m, Edge<Key, BlockMatrix<T>>& input) {
  auto f = [m](const Key &key) {
    auto [i, j] = key;
    std::cout << "Getting block " << i << ":" << j << std::endl;
    return m.at(Key(i,j));
  };
...
int main(int argc, char** argv) {
  ...
  Edge<Key, BlockMatrix<double>> input0("input0"),// input1BR("input1BR"), input2BR("input2BR"), input1R("input1R"),
    //  input2R("input2R"), input1B("input1B"), input2B("input2B"), input2L("input2L"),
    toporleft("toporleft"),
    toporleftLR("toporleftLR"), toporleftLB("toporleftLB"), output1("output1"), output2("output2"),
    output1R("output1R"), output1B("output1B"), output12R("output12R"), output12B("output12B"), outputLB("outputLB"),
      outputLR("outputLR"), output2R("output2R"), output2B("output2B"), output2TL("output2TL"), output2LL("output2LL"),
      result("result");
  Edge<Key, BlockMatrix<double>> bottom0("bottom0", true, get_bottomindex_func);
  Edge<Key, BlockMatrix<double>> right0("right0", true, get_rightindex_func);

  Edge<Key, BlockMatrix<double>> block("block", true, get_inputindex_func);
  ...
  auto b1 = make_get_inputblock(m, bottom0);
  auto r1 = make_get_inputblock(m, right0);
  ...
  //  i->invoke(Key(my_rank * local_row_count, 0));
  //}
