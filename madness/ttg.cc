...
class Everything {
 ...
 public:
  Everything() : producer("producer"), a("A"), consumer("consumer"), world(::madness::ttg::get_default_world()) {
  ...
  void start() {
    if (world.rank() == 0) producer.invoke(0);
  }
...
class EverythingBase {
 ...
 public:
  ...
  void start() {
    if (world.rank() == 0) dynamic_cast<Producer*>(producer.get())->invoke(0);
  }  // Ugh!
...
class Everything3 {
 ...
 public:
  ...
  void start() {
    if (::madness::ttg::get_default_world().rank() == 0) wp->invoke(0);
  }
