...
struct tripleT {
  ...
  void serialize(Archive& ar) {
    madness::archive::wrap((unsigned char*)this, sizeof(*this));
  }
};
...
class MxM {
  ...
  class BroadcastA : public Op<pairT, std::tuple<Out<tripleT, Matrix>>, BroadcastA, Matrix> {
   ...
   public:
    BroadcastA(std::size_t N, std::size_t M, std::size_t K, std::size_t tilesize, Edge<pairT, Matrix>& in,
               Edge<tripleT, Matrix>& out)
  ...
  class BroadcastB : public Op<pairT, std::tuple<Out<tripleT, Matrix>>, BroadcastB, Matrix> {
   ...
   public:
    BroadcastB(std::size_t N, std::size_t M, std::size_t K, std::size_t tilesize, Edge<pairT, Matrix>& in,
               Edge<tripleT, Matrix>& out)
  ...
  class MxMTask : public Op<tripleT, std::tuple<Out<tripleT, Matrix>>, MxMTask, Matrix, Matrix> {
   ...
   public:
    MxMTask(std::size_t N, std::size_t M, std::size_t K, std::size_t tilesize, Edge<tripleT, Matrix>& Aik,
            Edge<tripleT, Matrix>& Bkj, Edge<tripleT, Matrix>& Cijreduce)
  ...
  class Reduce : public Op<tripleT, std::tuple<Out<pairT, Matrix>, Out<tripleT, Matrix>>, Reduce, Matrix, Matrix> {
   ...
   public:
    Reduce(std::size_t N, std::size_t M, std::size_t K, std::size_t tilesize, Edge<tripleT, Matrix>& Cijpartial,
           Edge<pairT, Matrix>& result)
        : baseT("reduce", {"Cij partial", "sum"}, {"Cij total", "sum"}), N(N), M(M), K(K), tilesize(tilesize) {
    ...
    void op(tripleT ijk, const std::tuple<Matrix, Matrix>& t,
            std::tuple<Out<pairT, Matrix>, Out<tripleT, Matrix>>& out) {
 ...
 public:
  MxM(std::size_t N, std::size_t M, std::size_t K, std::size_t tilesize, Edge<pairT, Matrix>& Aik,
      Edge<pairT, Matrix>& Bkj, Edge<pairT, Matrix>& Cij)
      : N(N)
      , M(M)
      , K(K)
      , tilesize(tilesize)
...
class Writer : public Op<pairT, std::tuple<>, Writer, Matrix> {
 ...
 public:
  Writer(Edge<pairT, Matrix>& in, Matrix& A, std::size_t tilesize)
      : baseT(edges(in), edges(), "writer", {"Matrix tile"}, {}), A(A), tilesize(tilesize) {}
...
class ReadA : public Op<int, std::tuple<Out<pairT, Matrix>>, ReadA, int> {
 ...
 public:
  ReadA(const Matrix& A, std::size_t tilesize, Edge<int, int>& ctl, Edge<pairT, Matrix>& out)
      : baseT(edges(ctl), edges(out), "read A", {"ctl"}, {"Aik"}), A(A), tilesize(tilesize) {}
...
class ReadB : public Op<int, std::tuple<Out<pairT, Matrix>>, ReadB, int> {
 ...
 public:
  ReadB(const Matrix& B, std::size_t tilesize, Edge<int, int>& ctl, Edge<pairT, Matrix>& out)
      : baseT(edges(ctl), edges(out), "read B", {"ctl"}, {"Bkj"}), B(B), tilesize(tilesize) {}
...
class Control : public Op<int, std::tuple<Out<int, int>>, Control> {
 ...
 public:
  ...
  void op(const int& key, const std::tuple<>& junk, std::tuple<Out<int, int>>& out) { ::send<0>(0, 0, out); }
