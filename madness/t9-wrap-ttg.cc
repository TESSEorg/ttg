...
auto make_project(const funcT& func, ctlEdge& ctl, nodeEdge& result, const std::string& name="project") {
    ...
    return wrap(f,
                edges(fuse(refine,ctl)),
                edges(refine,result),
                name,{"control"},{"refine","result"});
}

template <typename funcT>
auto make_binary_op(const funcT& func, nodeEdge left, nodeEdge right, nodeEdge Result, const std::string& name="binaryop") {
    ...
    nodeEdge L("L"), R("R");
    return wrap(f,
                edges(fuse(left,L),fuse(right,R)),
                edges(L,R,Result),
                name,{"left","right"},{"refineL","refineR","result"});
}
...
void diff(const Key& key, const Node& left, const Node& center, const Node& right, std::tuple<nodeOut,nodeOut,nodeOut,nodeOut>& out) {
...
auto make_diff(nodeEdge in, nodeEdge out, const std::string& name="diff") {
    ...
    return std::make_tuple(wrap(&send_to_output_tree, edges(in), edges(L,C,R), "send_to_output_tree",{"input"},{"L","C","R"}),
...
auto make_compress(const nodeEdge& in, nodeEdge& out, const std::string& name="compress") {
    doubleEdge L("L"), R("R");
    return std::make_tuple(wrap(&send_leaves_up, edges(in), edges(L,R,out), "send_leaves_up", {"input"}, {"L","R","result"}),
...
public:
    Norm2(const nodeEdge& in, const std::string& name="norm2") : baseT(edges(in), edges(), name, {"nodes"}, {}), sumsq(0.0) {}
    ...
    void op(const Key& key, const std::tuple<Node>& t, std::tuple<>& output) {
        madness::ScopedMutex<madness::SCALABLE_MUTEX_TYPE> obolus(charon); // <<<<<<<<<< mutex
